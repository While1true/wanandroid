...
class ArticalState extends State<ArticalItem> {
  ...
  Widget build(BuildContext context) {
    return  Card(
      child: InkWell(
        onTap: _onTap, child:Stack(children: <Widget>[
      Padding(padding: EdgeInsets.all(16.0), child:
      new Column(crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Padding(padding: EdgeInsets.only(right: 32.0),
            child: Hero(tag:widget.artical.title,child:Text(escapeFromHtml(widget.artical.title), softWrap: true,
                maxLines: 2,
                style: TextStyle(fontSize: 17.0,
                    color: Color(0xff353535))),)),
          widget.artical.envelopePic == "" ? Padding(
            padding: EdgeInsets.all(1.0),) : Image.network(
            widget.artical.envelopePic,height: 200.0,),
  ...
  List<Widget> _buildWidge() {
    ...
    if (widget.artical.fresh) {
      widges.add(DecoratedBox(
          child: Padding(padding: EdgeInsets.all(1.0),
            child: Text("新", style: TextStyle(fontSize: 12.0),),),
    ...
    widges.add(GestureDetector(
      child: Text(widget.artical.superChapterName, style: TextStyle(
    ...
    widges.add(GestureDetector(
      child: Text(widget.artical.chapterName, style: TextStyle(
          color: Colors.blueGrey, fontStyle: FontStyle.italic, fontSize: 12.0)),
  ...
  void _onTap() {
    Navigator.of(context).push(MaterialPageRoute(builder: (context)=>MyWebPage(widget.artical.link,widget.artical.title)));
  }

  void _onCategoryClick() {
Scaffold.of(context).showSnackBar(SnackBar(content: Text("还未实现该功能"),));
  }
